...
router.post('/', [
    ...
    body('item_description').notEmpty().withMessage('Item description is required'),
    body('item_weight').optional().isFloat({ min: 0 }).withMessage('Weight must be positive'),
    body('item_value').optional().isFloat({ min: 0 }).withMessage('Value must be positive'),
    ...
    try {
        ...
        const tracking_id = 'DST' + Date.now() + Math.random().toString(36).substr(2, 5).toUpperCase();

        const result = await sql`
            INSERT INTO delivery_requests (
                user_id, pickup_address, pickup_lat, pickup_lng, delivery_address, delivery_lat, delivery_lng,
                item_description, item_weight, item_value, package_type, urgent, preferred_time, max_price, notes, tracking_id
            )
            VALUES (
                ${user_id}, ${pickup_address}, ${pickup_lat}, ${pickup_lng}, ${delivery_address}, ${delivery_lat}, ${delivery_lng},
                ${item_description}, ${item_weight}, ${item_value}, ${package_type}, ${urgent}, ${preferred_time}, ${max_price}, ${notes}, ${tracking_id}
        ...
        res.status(500).json({
            success: false,
            message: 'Server error creating delivery request'
        });
    }
});
...
router.get('/', async (req, res) => {
    try {
        ...
        let query = `SELECT dr.*, u.username as user_name, u.full_name, u.phone,
                     r.username as rider_name, r.full_name as rider_full_name, r.phone as rider_phone,
                     r.vehicle_type, r.rating as rider_rating
        ...
        if (user_id) {
            conditions.push(`dr.user_id = $${params.length + 1}`);
            params.push(user_id);
        }
        
        if (rider_id) {
            conditions.push(`dr.rider_id = $${params.length + 1}`);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await sql`
            SELECT dr.*, u.username as user_name, u.full_name, u.phone, u.email,
        ...
        res.status(500).json({
            success: false,
            message: 'Server error fetching delivery'
        });
    }
});
...
router.patch('/:id/status', [
    body('status').isIn(['pending', 'accepted', 'picked_up', 'delivered', 'cancelled']).withMessage('Invalid status'),
